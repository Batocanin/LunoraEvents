...
interface PartyCreateDesignPreviewProps {
  formData: PartyValues;
  mainPhotoUrl: string | undefined;
  backgroundPhotoUrl: string | undefined;
}

function PartyCreateDesignPreview({
  formData,
  mainPhotoUrl,
  backgroundPhotoUrl,
}: PartyCreateDesignPreviewProps) {
  return (
    <div>
          ...
          <div className="relative flex flex-col overflow-y-auto w-full h-full max-w-xl mx-auto shadow-lg">
            ...
            {formData?.message && (
              <div className="drop-shadow-lg p-3 space-y-2 pb-6">
                ...
                <p className="break-words">{formData.message}</p>
              </div>
            )}
          </div>
        </div>
...
export default PartyCreateDesignPreview;
